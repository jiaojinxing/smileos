...
*********************************************************************************************************/
/*********************************************************************************************************
** C++ 全局对象构建与析构函数表开始指针 (为了和一些编译器不产生冲突, 这里使用 SmileOS 自带的符号)
*********************************************************************************************************/
typedef void (*VOIDFUNCPTR)(void);

static VOIDFUNCPTR __MY_CTOR_LIST__[1] __attribute__((section(".ctors"))) = { (VOIDFUNCPTR)-1 };
static VOIDFUNCPTR __MY_DTOR_LIST__[1] __attribute__((section(".dtors"))) = { (VOIDFUNCPTR)-1 };
/*********************************************************************************************************
** Function name:           __cppRtDoCtors
** Descriptions:            C++ 运行全局对象构造函数
...
void __cppRtDoCtors(void)
{
    ...
    for (ppfunc = __MY_CTOR_LIST__ + 1; *ppfunc != NULL; ppfunc++) {
...
/*********************************************************************************************************
** Function name:           __cppRtDoDtors
** Descriptions:            C++ 运行全局对象析构函数
** input parameters:        NONE
** output parameters:       NONE
** Returned value:          NONE
*********************************************************************************************************/
void __cppRtDoDtors(void)
{
    ...
    for (ppfunc = __MY_CTOR_LIST__ + 1; *ppfunc != NULL; ppfunc++);     /*  首先需要运行最后一个        */
    ppfunc--;

    while (ppfunc > __MY_DTOR_LIST__) {
        if ((*ppfunc) != (VOIDFUNCPTR)-1) {                             /*  可能 C 库还有一个起始符号   */
            (*ppfunc)();
        }
        ppfunc--;
