...
**
** 再重新打成一个新的 emWin 库, 我需要重新实现下面的函数了, 由于没文档可参考, 也没反汇编看它的具体实现,
**
** 我不确定最后几个函数的实现是否有问题
**
**--------------------------------------------------------------------------------------------------------
...
GUI_HMEM           GUI_ALLOC_Realloc         (GUI_HMEM hOld, int NewSize)
{
    ...
    if (hOld) {
        hNew = GUI_ALLOC_AllocInit((const void *)hOld, GUI_ALLOC_GetSize(hOld));
        GUI_ALLOC_Free(hOld);
    } else {
