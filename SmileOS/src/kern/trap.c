...
void dabt_c_handler(uint32_t *regs)
{
    ...
    case 7:
        if (current->pid != 0) {                                        /*  如果是在进程里发生错误      */
             ...
             *
             * 而 qemu-system-arm.exe 有 bug, 返回的始终是 VA
             *
             * 所以加上下面的修正代码
             */
            ...
            pid = MVA / PROCESS_SPACE_SIZE;                             /*  计算 PID                    */

            if (pid != 0 && pid < PROCESS_NR) {                         /*  如果 PID 有效               */

                task_t *task = &tasks[pid];                             /*  获得任务控制块              */

                if (task->status != TASK_UNALLOCATE) {                  /*  如果进程有效                */
