...
** 创   建   人: MakeSymbol 工具										
**														
** 文件创建日期: 2012 年 08 月 02 日						
**														
** 描        述: SmileOS 操作系统符号表. (此文件由 MakeSymbol 工具自动生成, 请勿修改)			
*********************************************************************************************************/	
#include "kern/symbol_tool.h"										
														
#define SYMBOL_TABLE_BEGIN  SYMBOL smileos_symbol_table[] = {
...
extern int  __switch_to(); 
extern int  __switch_to_process0(); 
extern int  __swrite(); 
...
extern int  file_free(); 
extern int  file_manager_init(); 
...
extern int  interrupt_exit_no_sched(); 
...
extern int  scalbnl(); 
extern int  scanf(); 
extern int  sd_read_block(); 
extern int  sd_write_block(); 
extern int  sdcard_create(); 
extern int  seed48(); 
extern int  seekdir(); 
extern int  select(); 
...
extern int  optopt; 
extern int  rootfs; 
extern int  s3c2440_sdio; 
extern int  sdcard_drv; 
...
SYMBOL_TABLE_BEGIN												
    ...
    SYMBOL_ITEM_FUNC(__switch_to) 
    SYMBOL_ITEM_FUNC(__switch_to_process0) 
    ...
    SYMBOL_ITEM_FUNC(file_alloc) 
    SYMBOL_ITEM_FUNC(file_free) 
    SYMBOL_ITEM_FUNC(file_manager_init) 
    ...
    SYMBOL_ITEM_FUNC(interrupt_exit_no_sched) 
    ...
    SYMBOL_ITEM_FUNC(kgetc) 
    SYMBOL_ITEM_FUNC(kheap_create) 
    SYMBOL_ITEM_FUNC(kheap_show) 
    ...
    SYMBOL_ITEM_FUNC(netconn_write) 
    ...
    SYMBOL_ITEM_FUNC(sd_read_block) 
    SYMBOL_ITEM_FUNC(sd_write_block) 
    SYMBOL_ITEM_FUNC(sdcard_create) 
    ...
    SYMBOL_ITEM_FUNC(vfs_process_fork) 
    ...
    SYMBOL_ITEM_FUNC(vmm_process_fork) 
    SYMBOL_ITEM_FUNC(vmm_process_init) 
    SYMBOL_ITEM_FUNC(vmm_show) 
    ...
    SYMBOL_ITEM_OBJ(s3c2440_sdio) 
    SYMBOL_ITEM_OBJ(sdcard_drv) 
