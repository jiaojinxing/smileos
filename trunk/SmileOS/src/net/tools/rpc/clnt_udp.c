...
/********************************************* jiaojinxing **********************************************/
/*
 * 由于 lwip 缺少 getifaddrs 函数, 所以重新实现 is_network_up 函数.
 */
#ifdef SMILEOS

int is_network_up(int sock)
{
    struct netif *_netif = netif_find(DEFAULT_NETIF_NAME);

    return  (netif_is_up(_netif) && netif_is_link_up(_netif));
}
