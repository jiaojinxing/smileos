...
static void dm9000_rx_status_32bit(u16_t *status, u16_t *len)
{
    ...
    *status = le16_to_cpu(temp);
    *len    = le16_to_cpu(temp >> 16);
}
/*********************************************************************************************************
...
static void dm9000_rx_status_16bit(u16_t *status, u16_t *len)
{
    ...
    *status = le16_to_cpu(DM9000_inw(DM9000_DATA));
    *len    = le16_to_cpu(DM9000_inw(DM9000_DATA));
}
/*********************************************************************************************************
...
static void dm9000_rx_status_8bit(u16_t *status, u16_t *len)
{
    ...
    *status = le16_to_cpu(DM9000_inb(DM9000_DATA) + (DM9000_inb(DM9000_DATA) << 8));
    *len    = le16_to_cpu(DM9000_inb(DM9000_DATA) + (DM9000_inb(DM9000_DATA) << 8));
}
/*********************************************************************************************************
...
static void dm9000_isr(struct netif *netif)
{
    ...
    if (int_status & ISR_PTS) {
        workqueue_add(NULL, (void (*)(void *))dm9000_send_done, netif);
        /*
         * 关闭发送中断
         */
        reg &= ~(IMR_PTM);
